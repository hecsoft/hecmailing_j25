...
{ 
    ...
    function _buildQuery()
   {
      ...
      if ($group>0) {
          $where[] = "l.grp_id_groupe =".$group;
        }
      if (!$isadmin)
      {
      	$where[] = "l.usr_id_user=".$user->id;
      }
     
		  if (isset($filter_order) && $filter_order!='')
		  {
  ...
  function getData($limitstart,$limit) 
  {
        ...
        if (empty($this->_data)) {
            $query = $this->_buildQuery();
            //echo $query;
            //echo $this->getState('limitstart').";". $this->getState('limit');
 ...
 function getTotal()
   ...
   function getGroups()
  {
      ...
      $groupes=array();
      $groupes_html=array();
      $groupes_html["0"] = JHTML::_('select.option', '0', "{".JText::_( 'ALL' )."}", 'grp_id_groupe', 'grp_nm_groupe');
      foreach ($rows as $r)
      {
